...
import {markdownify, Link, toUrl, safePrefix, classNames, htmlToReact} from '../utils';
                                ...
                                {_.get(this.props, 'pageContext.site.data.footer.about.actions') && 
                                    <ul className="actions">
                                        {_.map(_.get(this.props, 'pageContext.site.data.footer.about.actions'), (action, action_idx) => (
                                            <li key={action_idx}><Link to={(_.get(action, 'url').startsWith('#') ? _.get(action, 'url') : safePrefix(toUrl(this.props.pageContext.pages, _.get(action, 'url'))))} className={classNames('button', {'primary': _.get(action, 'is_primary')}, {'scrolly': _.get(action, 'is_scrolly')})}>{_.get(action, 'label')}</Link></li>
                                        ))}
                                    </ul>
                                }
                            </section>
